...
function syncImportedMaterials(state: LogisticsState, event: DomainEvent) {
  ...
  notify(state, { type: "material_imported", priority: "alta", entity_type: "catalog", entity_id: event.entityId || event.id, href: "/inventario", message: `Nueva base de datos importada - ${created} referencias nuevas, ${updated} actualizadas` });
...
function routeServiceIncident(state: LogisticsState, event: DomainEvent) {
  ...
  notify(state, { type: "cross_module_incident", priority: "critica", entity_type: "incident", entity_id: incident.id, href: `/incidencias?id=${incident.id}`, message: `Incidencia logística desde ${event.originModule}: ${incident.codigo}` });
