  ...
  return (
    <section className="rounded-2xl border bg-white p-4">
      ...
      <div className="mt-3 grid gap-2 text-sm md:grid-cols-2">
        <Read label="Picking" value={picking ? <a href={`/picking?id=${picking.id}`} className="font-semibold underline-offset-2 hover:underline">{picking.codigo}</a> : "Sin picking"} />
        <Read label="Envío" value={shipment ? <a href={`/envios?id=${shipment.id}`} className="font-semibold underline-offset-2 hover:underline">{shipment.transportista || "Transporte"} · {shipment.tracking || shipment.estado}</a> : "Sin envío"} />
        <Read label="Incidencia activa" value={incident ? <a href={`/incidencias?id=${incident.id}`} className="font-semibold underline-offset-2 hover:underline">{incident.codigo} · {incident.tipo}</a> : "Sin incidencia"} />
        <Read label="Pendiente llegada" value={pending ? <a href={`/incidencias?id=${pending.id}`} className="font-semibold underline-offset-2 hover:underline">{pending.fecha_prevista || pending.estado}</a> : "Sin pendiente"} />
      </div>
    </section>
  );
}

function Read({ label, value }: { label: string; value: React.ReactNode }) { return <div className="rounded-xl bg-white p-3"><p className="text-xs font-semibold text-slate-500">{label}</p><div>{value}</div></div>; }
function sourceLabel(sourceType?: string, sourceId?: string | null) { const labels: Record<string, string> = { service: "Servicio", service_point: "Punto de servicio", isdin_vinyl: "ISDIN Vinilos", campaign: "Campaña", incident: "Incidencia", replacement: "Reposición", manual_request: "Petición manual" }; return [sourceType ? labels[sourceType] || sourceType : "", sourceId].filter(Boolean).join(" · "); }
function firstText(...values: Array<string | number | null | undefined>) { return values.map(value => String(value ?? "").trim()).find(Boolean) || ""; }
function logisticVinLabel(status: string) { const labels: Record<string, string> = { pendiente_recepcion: "Pendiente recepción", pendiente_picking: "Pendiente picking", en_almacen: "En almacén", en_picking: "En picking", enviado: "En tránsito", entregado: "Entregado", con_incidencia: "Con incidencia", bloqueado: "Bloqueado", cancelado: "Cancelado" }; return labels[status] || status; }
function statusClass(label: string) { const text = label.toLowerCase(); const tone = text.includes("incid") || text.includes("bloque") ? "bg-red-50 text-red-800 ring-red-200" : text.includes("tránsito") || text.includes("picking") ? "bg-blue-50 text-blue-800 ring-blue-200" : text.includes("entregado") ? "bg-emerald-50 text-emerald-800 ring-emerald-200" : "bg-amber-50 text-amber-800 ring-amber-200"; return `inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 ${tone}`; }
